...
""" After I created my app models and added them to the settings->INSTALLED_APPS,
    ...
    Interacting with the Database
        ...
        Then run these to save a new post:
            ...
            >>> from coreProject.blog.models import Post

            The get() method allows you to retrieve a single object from the database. Note that this method
            expects a result that matches the query:
                - If no results are returned by the database: return DoesNotExist exception
                - If the database returns more than one result: return MultipleObjectsReturned exception
            >>> user = User.objects.get(username='artinmac')
            ...
            Deleting Objects: Note that deleting objects will also delete any dependent relationships 
                              for ForeignKey objects defined with on_delete set to CASCADE.
                >> Post.objects.get(id=1).delete()
                >> Post.objects.filter(title__startswith='Another post').delete()
